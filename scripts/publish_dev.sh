...
echo "Determine the next prerelease version (ex. 1.2.1-dev.0)"
NEXT_PRE_VERSION=$(npx semver -i prerelease $CURRENT_VERSION --preid $PREID)
echo "👉 $NEXT_PRE_VERSION"

echo "Determine the next release version (ex. 1.2.1)"
NEXT_VERSION=$(npx semver -c $NEXT_PRE_VERSION)
echo "👉 $NEXT_VERSION"
